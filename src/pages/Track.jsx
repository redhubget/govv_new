...
// --- Small layout helpers (match your app’s styles)
const Card = ({ title, children, className = "" }) => (
  <div className={`card ${className}`}>
    {title && <div className="text-sm" style={{ color: 'var(--muted)', marginBottom: 8, letterSpacing: '.02em' }}>{title}</div>}
    {children}
  </div>
)
const Button = ({ children, onClick, variant = "primary", disabled }) => {
  ...
  const styles = variant === "primary"
    ? {} // uses --accent in global button styles
    : variant === "ghost"
...
export default function Track() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {}, // ignore error (stay on fallback)
      { enableHighAccuracy: true, maximumAge: 15000, timeout: 10000 }
    )
  }, [])

  // keep ref for async interval
  ...
  const stopTracking = async () => {
    ...
    if (currentPath.length < 2) {
      // reset
      setPath([]); setDistance(0); setAvgSpeed(0); setStartTime(null)
      return
    }
    ...
    // here you could POST to your backend if desired
    // await axios.post(`${API}/activities`, { ... })

    // reset + navigate to history (optional)
    setStartTime(null)
    pausedDurationRef.current = 0
    navigate('/activities')
  }
  ...
  const durationSec = useMemo(() => {
    ...
    return Math.max(0, Math.floor((Date.now() - startTime - pausedDurationRef.current) / 1000))
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card title={isTracking ? (isPaused ? "Paused" : "Live Ride") : "Ready to Ride"}>
            <div className="map-wrap">
              <MapContainer
                ...
                style={{ height: '100%', borderRadius: 16, border: '1px solid var(--border)' }}
                zoomControl={false}
              >
                <TileLayer
        ...
        <div className="space-y-4">
          <Card title="Ride Tips">
            <ul style={{ color: 'var(--text)', opacity: .85, margin: 0, paddingLeft: 18 }}>
